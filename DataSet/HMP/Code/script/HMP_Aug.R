...
for (IT in 1:Total_split){
  ...
  for (i in 1:length(percentage)){
    ...
    x0 <- x0[-1,];
    labels <- labels[-1,];
    nb_sample_file <- paste("/Users/chiehlo/Desktop/HMP_project/deep/datasets/HMP/Data/Augmentation/IT", IT, "/nbSample_",  IT , "_", round(percentage[i]*100),  ".txt", sep = "");
    nb_label_file <- paste("/Users/chiehlo/Desktop/HMP_project/deep/datasets/HMP/Data/Augmentation/IT", IT, "/nbLabel_", IT, "_", round(percentage[i]*100), ".txt", sep = "");
  ...
  for (i in 1:length(add_on)){
    ...
    x0 <- x0[-1,];
    labels <- labels[-1,];
    nb_sample_file <- paste("/Users/chiehlo/Desktop/HMP_project/deep/datasets/HMP/Data/Augmentation/IT", IT, "/nbSample_",  IT , "_", round(add_on[i]*nClasses),  ".txt", sep = "");
    nb_label_file <- paste("/Users/chiehlo/Desktop/HMP_project/deep/datasets/HMP/Data/Augmentation/IT", IT, "/nbLabel_", IT, "_", round(add_on[i]*nClasses), ".txt", sep = "");
    write.table(x0, nb_sample_file, sep="\t", row.names = F, col.names = F)
    write.table(labels, nb_label_file, sep="\t", row.names = F, col.names = F)
  }
