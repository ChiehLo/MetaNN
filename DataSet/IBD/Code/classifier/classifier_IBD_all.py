...
def fit_classifier(X, y, ctype, n_features, index):
	...
	if ctype == 'RF':
		...
		clf = MLPClassifier(solver='adam', alpha=0.0001, max_iter=1000, learning_rate='adaptive', hidden_layer_sizes=(512,256,128,),  learning_rate_init=1e-3, activation='relu').fit(X_new, y)
	elif ctype == 'MB':
...
def read_files(IT, dataset):
	...
	temp = np.zeros((train_label.shape[0],1))
	for i in range(train_label.shape[0]):
		temp[i] = np.where(train_label[i,:] == 1)
	train_label = temp.astype(int).ravel()
...
run_times = 1
ACC = np.array([0, 0, 0, 0, 0]).astype(float)
